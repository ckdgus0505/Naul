...
namespace Interface
{
    public partial class Form1 : Form
    {
        ...
        {
            string constring = "Server=isg1031.iptime.org;port=20050;Database=ai;username=aitester;password=aitester001!";
            MySqlConnection connection = new MySqlConnection(constring);    //DB연결객체
            MySqlCommand cmdDataBase = new MySqlCommand(" SELECT * FROM attendance;", connection);
            ...
            try
            {
                ...
                using (MySqlDataReader reader = cmdDataBase.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        string[] row = { reader[0].ToString(), reader[1].ToString(), reader[3].ToString(), reader[2].ToString() };
